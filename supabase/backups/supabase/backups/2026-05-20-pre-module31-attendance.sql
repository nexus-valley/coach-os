...
CREATE TABLE public.attendance_records (
  ...
  CONSTRAINT attendance_records_session_id_fkey FOREIGN KEY (session_id) REFERENCES public.sessions(id),
  CONSTRAINT attendance_records_student_id_fkey FOREIGN KEY (student_id) REFERENCES public.students(id),
  CONSTRAINT attendance_records_marked_by_fkey FOREIGN KEY (marked_by) REFERENCES auth.users(id)
);
...
CREATE TABLE public.invoice_items (
  ...
  tax_percent numeric NOT NULL DEFAULT 0 CHECK (tax_percent >= 0::numeric),
...
CREATE TABLE public.invoices (
  ...
  status text NOT NULL DEFAULT 'draft'::text CHECK (status = ANY (ARRAY['draft'::text, 'issued'::text, 'paid'::text, 'overdue'::text, 'void'::text])),
...
CREATE TABLE public.payment_transactions (
  ...
  tenant_id uuid NOT NULL,
  invoice_id uuid,
  provider text NOT NULL DEFAULT 'manual'::text CHECK (provider = ANY (ARRAY['razorpay'::text, 'stripe'::text, 'manual'::text])),
...
CREATE TABLE public.sessions (
  ...
  CONSTRAINT sessions_cohort_id_fkey FOREIGN KEY (cohort_id) REFERENCES public.cohorts(id),
  CONSTRAINT sessions_trainer_user_id_fkey FOREIGN KEY (trainer_user_id) REFERENCES auth.users(id),
...
CREATE TABLE public.subscriptions (
  ...
  status text NOT NULL DEFAULT 'trialing'::text CHECK (status = ANY (ARRAY['trialing'::text, 'active'::text, 'past_due'::text, 'canceled'::text, 'expired'::text])),
...
CREATE TABLE public.team_invitations (
  ...
  tenant_id uuid NOT NULL,
  email text NOT NULL,
  role text NOT NULL CHECK (role = ANY (ARRAY['admin'::text, 'staff'::text, 'trainer'::text])),
  ...
  CONSTRAINT team_invitations_tenant_id_fkey FOREIGN KEY (tenant_id) REFERENCES public.tenants(id),
  CONSTRAINT team_invitations_invited_by_fkey FOREIGN KEY (invited_by) REFERENCES auth.users(id),
  CONSTRAINT team_invitations_accepted_by_fkey FOREIGN KEY (accepted_by) REFERENCES auth.users(id)
);
...
CREATE TABLE public.tenants (
  ...
  plan_limits_json jsonb NOT NULL DEFAULT '{}'::jsonb,
