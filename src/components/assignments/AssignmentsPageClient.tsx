...
export function AssignmentsPageClient() {
  ...
  async function loadAssignmentContext(currentTenant: Tenant) {
    const [tenantAssignments, tenantCourses, tenantCohorts] = await Promise.all([
      getAssignments(currentTenant.id),
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        setError(getErrorMessage(caught, "Unable to load assignments."));
      } finally {
        if (active) {
          setLoading(false);
        }
  ...
  async function refreshAssignments() {
    ...
    await loadAssignmentContext(tenant);
  }
  ...
  async function handleCreateAssignment(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setError(getErrorMessage(caught, "Unable to create assignment."));
    } finally {
      setSaving(false);
    }
  }
    ...
    <div className="mx-auto max-w-7xl">
      <div className="flex flex-col justify-between gap-5 sm:flex-row sm:items-end">
        <div>
          ...
          <p className="mt-3 max-w-2xl text-base leading-7 text-[#425B76]">
            Create homework, track submissions, review student work, and build
            grading analytics for courses and cohorts.
          </p>
        </div>
        {canManage ? (
      ...
      {loading ? (
        ...
        <EmptyState
          ...
          description="Create a homework item for a course or cohort to start tracking submissions."
          icon="AS"
        ...
        <section className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {filteredAssignments.map((assignment) => {
            const submitted =
              assignment.submissionCounts.submitted +
              assignment.submissionCounts.reviewed +
              assignment.submissionCounts.late;
                ...
                <div className="mt-8 border-t border-[#D8E8F0] pt-5">
                  <div className="grid grid-cols-2 gap-3 text-sm">
                    <div className="rounded-2xl bg-[#F6FBFE] p-3">
                      ...
                      <p className="mt-1 text-xs text-[#66788F]">Submitted</p>
                    </div>
                    <div className="rounded-2xl bg-[#F6FBFE] p-3">
                      <p className="font-semibold text-[#0B1F33]">
                        {assignment.submissionCounts.reviewed}
                      </p>
                      <p className="mt-1 text-xs text-[#66788F]">Reviewed</p>
                    </div>
                  </div>
                  <div className="mt-5">
                    <Button href={`/app/assignments/${assignment.id}`} size="sm">
                      Open Assignment
                    </Button>
                  </div>
                </div>
            ...
            <form className="mt-7 space-y-5" onSubmit={handleCreateAssignment}>
              ...
              <label className="block">
                ...
                <textarea
                  ...
                  placeholder="Short context for this assignment."
