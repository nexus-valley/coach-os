...
export async function POST(request: Request, context: DownloadContext) {
  ...
  try {
    ...
    if (authorized.error) {
      const status = authorized.error.code === "02000" ? 404 : 403;
      return jsonError("Assignment file is unavailable.", status);
    }
    ...
    captureServerException(error, {
      ...
      operation: "assignment_attachment_download_unexpected",
      route: "/api/assignment-attachments/[attachmentId]/download",
    });
    return jsonError("Assignment file could not be opened.", 500);
  }
}
